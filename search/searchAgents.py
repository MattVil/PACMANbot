...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # need to be tuple and not array because array are not hashable
        # and we need to hash it to be compared in the State Object
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            if(not hitsWall):
                nextGameState = (nextx, nexty)
                remainingCorners = tuple(list(state[1]))
                if(nextGameState in state[1]):
                    remainingCorners = tuple(x for x in remainingCorners if x!=nextGameState)
                nextState = (nextGameState, remainingCorners)
